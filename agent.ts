...
export async function callAgent(client: MongoClient, query: string, thread_id: string) {
    try {
        ...
        // Explicitly type the input and output for the tool to avoid deep type inference issues
...
async function getData() {
    const query = "give me a Modern Coffee Table  price "
    ...
    const result = await callAgent(client, query, id)
    console.log(result)
}
getData()